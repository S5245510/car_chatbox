...
chroma/: (Automatically generated) Directory where the Chroma vector database is persisted.

get_embedding_function.py: Defines the function to get the embedding model.

populate_database.py: Script to load documents from data/, split them, generate embeddings, and populate/update the Chroma database.

query_data.py: Core script for performing RAG queries. It takes a question, retrieves relevant context from Chroma, and generates an answer using an LLM.

app.py: A Flask backend API that exposes the query_data.py functionality via an HTTP endpoint, allowing the web GUI to interact with it.

chatbot_gui.html: The frontend web application (HTML, CSS, JavaScript) providing the chatbot interface.

test_rag.py: Contains unit tests for the RAG system's functionality.
